...
export const api = axios.create({
  baseURL: "http://localhost:5000/api", // Note: auth.api.ts had /api appended, so let's stick to that convention or not?
  // Most routes are /api/...
  // If I set baseURL to .../api, then requests should be /auth/login, /dashboard/stats
  // But other files use /api/buildings...
  // Let's use http://localhost:5000 and include /api in requests, OR use /api in baseURL and remove it from requests.
  // The existing code has /api/ prefix in requests.
  // So baseURL should be http://localhost:5000
});
